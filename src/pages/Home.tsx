...
function Home() {
  ...
  const recentWorksSectionRef = useRef<HTMLElement>(null);
  const testimonialSectionRef = useRef<HTMLElement>(null);
  const getInTouchSectionRef = useRef<HTMLElement>(null);

  const aboutInView = useInView(aboutSectionRef, { once: true, amount: 0.2 });
  const recentWorksInView = useInView(recentWorksSectionRef, {
    once: true,
    amount: 0.2,
  });
  const testimonialInView = useInView(testimonialSectionRef, {
    once: true,
    amount: 0.2,
  });
  const getInTouchInView = useInView(getInTouchSectionRef, {
    once: true,
    amount: 0.2,
  });

  const springXY = { type: "spring" as const, stiffness: 100, damping: 20 };
  const springOpacity = { type: "spring" as const, stiffness: 50, damping: 15 };
    ...
    <div
      ...
      <section className="w-full space-y-14 md:space-y-18 gap-24 lg:gap-32 flex flex-col items-center justify-between">
        ...
        <motion.div
          ...
          <Button
            asChild
            className="min-[450px]:w-auto min-[320px]:w-[300px] hover:bg-primary/80 transition-all duration-300 cursor-pointer w-[calc(100dvw-20px)] bg-[rgba(139,92,246,1)] text-white min-[450px]:h-12 h-10 font-semibold min-[450px]:text-base text-sm">
            <Link to={"/projects"}>
              View My Work
              <ArrowRight />
            </Link>
          ...
          <SendMessageDialog>
            <Button
              ...
              <button className="text-sm hover:cursor-pointer font-semibold min-[650px]:flex hover:bg-primary/10 hover:text-primary hover:no-underline transition-all duration-300 cursor-pointer dark:text-secondary-foreground hover:dark:text-primary text-primary">
                Contact Me
              </button>
            </Button>
          </SendMessageDialog>
      ...
      <section
        ref={aboutSectionRef}
        className="w-full py-[4rem] flex md:flex-row flex-col items-center md:gap-[3rem] px-[0.5rem] sm:px-[1rem] md:px-[5rem] lg:px-[10rem] 2xl:px-[20rem]">
        <motion.div
          initial={{ x: -400, y: 400, opacity: 0 }}
          animate={aboutInView ? { x: 0, y: 0, opacity: 1 } : undefined}
          transition={{ x: springXY, y: springXY, opacity: springOpacity }}
        ...
        <div className="w-full flex flex-col items-center md:items-start py-6 gap-4 md:text-left text-center">
          <motion.div
            ...
            animate={
              aboutInView ? { x: 10, y: -20, opacity: 1 } : undefined
            }
            transition={{ x: springXY, y: springXY, opacity: springOpacity }}
      ...
      <section
        ref={recentWorksSectionRef}
        className="w-full h-auto py-[4rem] pt-[1rem] md:pt-[8rem] md:py-[8rem] flex lg:flex-row flex-col 2xl:gap-[5rem] lg:gap-[3rem] gap-[2rem] xl:gap-[6rem] items-center min-[750px]:px-0 px-[0.5rem] lg:pl-[5rem] xl:pl-[10rem] min-[1809px]:pl-[20rem] min-[1809px]:px-[20rem]">
        <div className="flex flex-col items-center lg:items-start gap-6 2xl:w-[558px] 2xl:min-w-[558px] sm:w-[450px] sm:min-w-[450px] lg:text-left text-center">
          <motion.div
            initial={{ x: 0, y: -60 }}
            animate={recentWorksInView ? { x: 0, y: 0 } : undefined}
            transition={{ x: springXY, y: springXY }}
        ...
        <motion.div
          ...
          animate={
            recentWorksInView ? { x: 0, y: 0, opacity: 1 } : undefined
          }
          transition={{ x: springXY, y: springXY, opacity: springOpacity }}
      ...
      <section
        ...
        <motion.div
          ...
          animate={testimonialInView ? { y: 0, opacity: 1 } : undefined}
          transition={{ y: springXY, opacity: springOpacity }}
      ...
      <section
        ...
        className="w-full px-[0.5rem] gap-y-6 md:gap-0 flex flex-col items-center justify-between h-full space-y-20 mt-20">
        <motion.div
          initial={{ y: -300, opacity: 0 }}
          animate={getInTouchInView ? { y: 0, opacity: 1 } : undefined}
          transition={{
            delay: 1.0,
            ease: "circInOut",
            type: "spring",
        ...
        <SendMessageDialog>
          <motion.button
            ...
            animate={
              getInTouchInView ? { y: 0, x: 0, opacity: 1 } : undefined
            }
            transition={{
              delay: 2.0,
              ease: "circInOut",
        ...
        <AnimatedGroup
          ...
          <Link
            target="_blank"
            to="https://mail.google.com/mail/u/0/?fs=1&to=asantemiriam23@gmail.com&tf=cm"
            className="group hover:bg-primary transition-all duration-300 rounded-full w-10 h-10 bg-primary/10 flex justify-center items-center">
            <svg
              width="20"
              height="16"
