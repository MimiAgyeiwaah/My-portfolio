...
function Home() {
    ...
    useEffect(()=>{
        const interval = setInterval(() => {
            setActiveTestimonial(prev => prev === testimonial.length - 1 ? 0 : prev + 1)
    ...
    return (
        <div className='w-full min-h-full flex flex-col items-center'>
            <section className="w-full min-[650px]:min-h-[500px] py-[2rem] min-[650px]:py-[6rem] gap-[2rem] flex flex-col items-center justify-between">
                ...
                <div className="w-fullf flex flex-wrap items-center justify-center gap-4">
                    <Button className='min-[450px]:w-auto min-[320px]:w-[300px] w-[calc(100dvw-20px)] bg-primary text-white min-[450px]:h-12 h-10 font-semibold min-[450px]:text-base text-sm'>
                        View My Work
                        <ArrowRight/>
                    </Button>
                    <Button variant={'outline'} className='min-[450px]:w-auto min-[320px]:w-[300px] w-[calc(100dvw-20px)] text-primary border border-primary min-[450px]:h-12 h-10 font-semibold min-[450px]:text-base text-sm'>
                        Download Resume
                        <Download/>
                    </Button>
                    <a href="/" className='text-sm font-semibold min-[650px]:flex hidden'>Contact Me</a>
                </div>
            </section>
                ...
                <div className="w-full min-[750px]:w-[700px] lg:w-full h-[400px] xl:h-[490px] lg:flex-1 flex items-center relative overflow-hidden rounded-r-2xl sm:rounded-r-[3rem] lg:rounded-r-none rounded-l-2xl sm:rounded-l-[3rem]">
                    {
                        mockups.map((mockup, index) => (
                            <img key={index} src={mockup} alt="mock" className={`size-full absolute flex-1 object-cover ${activeMockup === index ? '' : 'translate-x-[95%] rounded-l-[3rem]'} ${activeMockup === index+1 ? 'z-10':''} transition-all duration-700`} />
                        ))
                    }
            ...
            <section className="w-full md:min-h-[100dvh] py-[4rem] md:py-[8rem] px-[0.5rem] flex flex-col items-center gap-[2rem] sm:gap-[5rem] bg-primary-foreground">
                <div className="w-fit flex flex-col gap-1 items-center">
                    <h1 className="font-semibold text-[30px] sm:text-[40px]">Client Testimonials</h1>
                    <hr className="w-[100px] border-2 border-primary" />
                    <p className='md:w-[600px] text-center text-gray-700 text-sm sm:text-base md:text-[18px] mt-3'>Here’s what clients and collaborators have to say about working with me.</p>
                </div>
                    ...
                    <section className="w-full flex md:flex-row flex-col items-center justify-between md:py-0 py-6">
                        <aside className="flex md:flex-row flex-col items-center gap-2">
                            <div className="w-[50px] h-[50px] rounded-full bg-gray-200 relative flex items-center justify-center">
                                ...
                                <img src={testimonial[activeTestimonial].image} alt="profilepicture" className="z-10 size-full object-cover rounded-full" />
                            </div>
                            <div className="flex flex-col">
                                <h1 className='font-semibold sm:text-[18px]'>{testimonial[activeTestimonial].name}</h1>
                                <p className='text-sm text-gray-500'>{testimonial[activeTestimonial].title}</p>
                            </div>
                        </aside>
                        <svg width="88" height="88" className='md:block hidden' viewBox="0 0 88 88" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M58.3365 45.3573H75.5332C75.2398 62.4806 71.8665 65.3039 61.3432 71.5373C60.1332 72.2706 59.7298 73.8106 60.4632 75.0573C61.1965 76.2673 62.7365 76.6706 63.9832 75.9373C76.3765 68.6039 80.7032 64.1306 80.7032 42.7906V23.0273C80.7032 16.7573 75.6065 11.6973 69.3732 11.6973H58.3732C51.9198 11.6973 47.0432 16.5739 47.0432 23.0273V34.0273C47.0065 40.4806 51.8832 45.3573 58.3365 45.3573Z" fill="#F0EAFE"/>
                        <path d="M18.6635 45.3573H35.8602C35.5668 62.4806 32.1935 65.3039 21.6702 71.5373C20.4602 72.2706 20.0568 73.8106 20.7902 75.0573C21.5235 76.2673 23.0635 76.6706 24.3102 75.9373C36.7035 68.6039 41.0302 64.1306 41.0302 42.7906V23.0273C41.0302 16.7573 35.9335 11.6973 29.7002 11.6973H18.7002C12.2102 11.6973 7.3335 16.5739 7.3335 23.0273V34.0273C7.3335 40.4806 12.2102 45.3573 18.6635 45.3573Z" fill="#F0EAFE"/>
                        </svg>
                    </section>
                    <p className='text-sm sm:text-base md:text-[18px] text-gray-700'>{testimonial[activeTestimonial].description}</p>
                </div>
                ...
                <div className="flex gap-6 items-center">
                    ...
                    <aside className="flex items-center gap-1">
                        <Circle className={`size-2 rounded-full ${activeTestimonial === 0 ? 'bg-primary' : 'bg-primary-foreground'} text-primary`}/>
                        <Circle className={`size-2 rounded-full ${(activeTestimonial > 0) && (activeTestimonial < (testimonial.length-1)) ? 'bg-primary' : 'bg-primary-foreground'} text-primary`}/>
                        <Circle className={`size-2 rounded-full ${activeTestimonial === testimonial.length-1 ? 'bg-primary' : 'bg-primary-foreground'} text-primary`}/>
                    </aside>
                    <button onClick={()=>{
            ...
            <section className="w-full md:h-[600px] py-[4rem] md:py-[8rem] pb-[3rem] px-[0.5rem] gap-[3rem] md:gap-0 flex flex-col items-center justify-between">
                    <div className="w-fit flex flex-col gap-1 items-center">
                        <h1 className="font-semibold text-[30px] sm:text-[40px]">Get In Touch</h1>
                        <hr className="w-[80px] border-2 border-primary" />
                        <p className='sm:w-[600px] text-center sm:text-base text-sm '>Let’s create something amazing together. Reach out for collaborations, question, or just to say hello. </p>
                    </div>
