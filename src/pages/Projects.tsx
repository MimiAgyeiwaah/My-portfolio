        ...
        <section className="w-fit grid sm:grid-cols-2 gap-4 md:gap-8">
          {works.map((work, idx) => {
            ...
            return (
              ...
              >
                <div
                  className="w-full hover:cursor-pointer min-[1261px]:w-[568px] min-[1261px]:h-[434px] overflow-hidden group relative flex items-center justify-center"
                  key={idx}
                >
                  <div
                    onClick={() => navigate(`/projects/${work.id}`)}
                    className="p-3 absolute rounded-lg group-hover:flex flex sm:hidden scale-75 group-hover:scale-100 transition-all duration-500 items-end sm:bg-white bg-gradient-to-br from-primary to-green-600 text-white group hover:bg-primary min-w-[180px] min-h-[70px] cursor-pointer sm:text-gray-600 hover:text-white z-10"
                  >
                    ...
                    <h1 className="group-hover:text-white">{work.title}</h1>
                  </div>
